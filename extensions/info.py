...
@lb.command("inrole2", "List of users in role", pass_options=True)
...
async def inrole_cmd(ctx: lb.Context, role: str) -> None:
    ...
    try:
        ...
        mem_id, mem_name = (member in ctx.bot.cache.get_members_view_for_guild(ctx.guild_id).values() if role.id in member.role_ids else ...)
